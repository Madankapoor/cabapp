...
func list(c *gin.Context) {
	...
	if cursor == "" {
		if err := db.Preload("User").Limit(10).Order("id desc").Find(&passenger).Error; err != nil {
		...
		if err := db.Preload("User").Limit(10).Order("id desc").Where(condition, cursor).Find(&passenger).Error; err != nil {
			c.AbortWithStatus(500)
			return
		}
	}

	length := len(passenger)
	serialized := make([]JSON, length, length)

	for i := 0; i < length; i++ {
		serialized[i] = passenger[i].Serialize()
	}
